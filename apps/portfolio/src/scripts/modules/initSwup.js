...
import { select, selectAll, selectByClass, selectById } from '../utils/select.js';
...
export function initSwup() {
    const swup = new Swup({
        ...
        plugins: [
            new SwupMorphPlugin({
                containers: ['#nav-links', '#menu-links']
            })
        ]
    });

    // Page transition container
    const ptcontainer = selectByClass('pt-container');
    const columns = selectAll('.pt-column');

    if (columns.length === 0) return;

    const initView = selectById('swup');
    ...
    swup.hooks.replace('animation:out:await', async () => {
        ...
        const liveView = selectById('swup');
